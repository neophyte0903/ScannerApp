...
url ='http://192.168.29.146:8080/shot.jpg'

while True:
    img=request.urlopen(url)
    imageBytes=bytearray(img.read())
    image_np=np.array(imageBytes,dtype=np.uint8)
    frame=cv2.imdecode(image_np,-1)
    frame_cvt=cv2.cvtColor(frame,cv2.COLOR_BGR2RGB)
    frameBlur=cv2.GaussianBlur(frame_cvt,(5,5),0)
    frame_edge=cv2.Canny(frameBlur,30,50)
